...
class ThreadedTCPRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        self.request.send(str('Client Got the File').encode())
        print('Global File recive completely!')
...
if __name__ == "__main__":
    host, port = "120.108.205.26", 50002 #不同client須改

    server = ThreadedTCPServer((host, port), ThreadedTCPRequestHandler)
    ip, port = server.server_address
    print('Client 1 Server start at: %s:%s' % (host, port))
    try:
